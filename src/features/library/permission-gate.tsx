...
/** Renders children only once video access is known; explains and requests it otherwise. */
export function PermissionGate({ children }: PropsWithChildren) {
  ...
  if (permission === 'unknown') {
    return <StateView loading title="Checking video access…" />;
  }
  ...
  if (permission === 'denied') {
    return canAskAgain ? (
      <StateView
        ...
        message="Player reads the videos stored on this phone to build your library. Nothing leaves your device."
        action={{ label: 'Allow access', onPress: requestAccess }}
      />
    ) : (
      <StateView
        icon="lock"
        title="Video access is turned off"
        message="Open Android settings and allow Player to access photos and videos."
  ...
  return (
    <View style={[styles.banner, { backgroundColor: theme.backgroundElement }]}>
      ...
      <ThemedText type="small" themeColor="textSecondary" style={styles.bannerText}>
        Player can only see the videos you selected.
      </ThemedText>
