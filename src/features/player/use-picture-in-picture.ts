...
export function usePictureInPicture(options: Options) {
  const { playing, paused, width, height, skipSeconds, onClosed, onTogglePlay, onSkip } = options;
  ...
  const onAction: NonNullable<VlcPlayerViewProps['onPictureInPictureAction']> = ({ nativeEvent }) => {
    if (nativeEvent.action === 'toggle') onTogglePlay();
    else onSkip((nativeEvent.action === 'rewind' ? -skipSeconds : skipSeconds) * 1000);
  };
